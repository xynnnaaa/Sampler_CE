...
class JoinSampler:
    def __init__(self, config_path: str):
        ...
        self.lookahead_k = samp_conf.get("lookahead_k", 2) 
        self.lookahead_samples = samp_conf.get("lookahead_samples", 100) 
    ...
    def _extract_lookahead_plan(self, start_node):
        plan =[]
        curr = start_node
        for _ in range(self.lookahead_k):
            ...
            curr = list(curr.children.values())[0] # Lookahead 取任一分支即可
        return plan
    ...
    def execute_root_step(self, root_node, partition_ids, pid_map, global_map, subtree_covered_mask):
        ...
        # root_node.beam = initial_candidates[:self.limit_x]

        # root不剪枝
        root_node.beam = initial_candidates

        return len(root_node.beam) > 0

    def execute_join_step(self, node, global_covered_mask, pid_map, global_map):
        parent_node = node.parent
        if not parent_node.beam: 
            node.beam =[]
            return False

        lookahead_plan = self._extract_lookahead_plan(node)
        target_mask = self._get_uncovered_mask(node.relevant_qids, global_covered_mask)
        ...
        for cand in candidates:
            t_idx, rj_data, rj_bmp, best_score = cand['t_idx'], cand['rj_data'], cand['rj_bmp'], cand['score']
            
            t_tuple = parent_node.beam[t_idx]
            new_real_bmp = t_tuple['bmp'] & rj_bmp
        ...
        node.beam =[{'data': d, 'bmp': b, 'score': s} for s, _, d, b in sorted_cands]
        
        return len(node.beam) > 0

    def dfs_join_recursive(self, current_node, subtree_covered_mask, pid_map, global_map):
        """
        深度优先遍历: 无回溯, 直接复用 parent.beam
        """
        collected_samples = defaultdict(list)
        newly_covered_global = 0

        # 执行当前节点扩展
        success = self.execute_join_step(current_node, subtree_covered_mask, pid_map, global_map)
        ...
        if current_node.is_template_end:
            sample_dict, covered_mask_update = self.collect_samples_for_node(current_node, subtree_covered_mask)
            if sample_dict:
                newly_covered_global |= covered_mask_update
                subtree_covered_mask |= covered_mask_update # 即时更新，影响兄弟分支的打分
    ...
    def collect_samples_for_node(self, node, current_covered_mask):
        if not node.beam: return None, 0
        best = node.beam[0]
            ...
            for p_idx, partition_ids in enumerate(partitions):
                ...
                if success:
                    if root_node.is_template_end:
                        s_dict, cov_mask = self.collect_samples_for_node(root_node, subtree_covered_mask)
                        if s_dict:
